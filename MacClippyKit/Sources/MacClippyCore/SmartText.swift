...
public struct MacClippyCaptureExclusionRules: Sendable, Equatable {
    ...
    // Apple Universal Clipboard / Handoff marker (nspasteboard.org). Never add
    // this to the concealed, transient, or auto-generated exclusion sets.
    public static let remoteClipboardPasteboardType = "com.apple.is-remote-clipboard"

    public static func containsRemoteClipboardMarker(_ types: some Sequence<String>) -> Bool {
        types.contains { $0 == remoteClipboardPasteboardType }
    }
    ...
    // Default product rules exclude concealed, transient, and autogenerated
    // UTIs plus built-in password-manager bundle IDs. Settings "Capture All"
    // turns those off. The observer applies these rules before Runtime sees a
    // change; the pasteboard write sentinel separately suppresses Mac Clippy's
    // own writes. legacyDefault() is a stricter historical preset, not the
